...
route.get('/banner',(req,res)=>{
    let data = req.productDATA.banner;
    res.send({
        code:0,
        msg: 'ok',
        data
...
route.get('/poster',(req,res)=>{
    let data = req.productDATA.poster;
    res.send({
        code:0,
        msg: 'ok',
        data
...
route.get('/info', (req, res) => {
    ...
    let {productId} = req.query;//=>GET请求问号传递信息都在REQ.QUERY上呢
    if(!productId) return;
    productId = parseFloat(productId);
    let item = req.productDATA.product_ph.find(item => {
...
route.get('/list', (req, res) => {
    ...
    if (type !== 'all') {
        ...
        req.productDATA.product_ph= req.productDATA.product_ph.filter(item => {
    ...
    let total = Math.ceil(req.productDATA.product_ph.length / limit),//=>总页数
        result = [];
    if (page <= total) {
        /*
        ...
        for (let i = (page - 1) * limit; i <= (page * limit - 1); i++) {
            let item = req.productDATA.product_ph[i];
            if (!item) break;
            result.push(item);
        }
    }
